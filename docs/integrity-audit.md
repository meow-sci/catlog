...
| **Shadow bans** — `shadowban` / `unshadowban` / `shadowban-delete`, the `shadowban_event` table | Moderation (constitution §7), and **not** the §8-forbidden mechanism of the same name. §8 forbids a machine inferring a cheater from the shape of their data and silently voiding them; this is an administrator naming an account for abuse or decency, with its log withheld rather than deleted so the call can be reviewed and reversed. Nothing about it reads a player's numbers. See [identity.md](identity.md) and constitution §8's note on the word |
