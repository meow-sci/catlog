...
| D12 | Repo layout | Top-level `mod/` (C#), `server/` (Go), `site/` (datastar assets + e2e), plus `contracts/`, `docs/`, `infra/`, `scripts/` (§2 — see [ARCHITECTURE.md](ARCHITECTURE.md)). Amended 2026-08-09 by [UI-057](#ui-057): the `spa/` React reader is gone and catlog has one frontend. |
...
### INGEST-025 — The conformance vectors cover all 23 types at their current `ver`, and the fixture set now fails when it stops doing so

*Accepted · 2026-08-09 · docs + fixtures.*

`contracts/testdata/batches/batch-001.ndjson` was five lines and five types, and covered none of the payload shapes the two implementations most easily disagree about — an optional present against the same optional absent, `kids` empty against populated, the `flight.ended` safety-net path. Both suites passed against it the whole time. **That is the failure mode worth naming: a conformance layer pinning a shape the implementations do not emit is worse than no conformance layer, because it reports green while the thing it guards has moved**, and the two teams reading it — Go and C# — each take it as evidence the other side agrees. The batch is now 27 lines covering **23 of 23** registry types at the `ver` each registry stamps, and `bh` moved to `9qWlcOUfLwdKlAHKyBSmoYe21h_-F48E-hUqLWC4WQg`, so `batch-001.br` and the four proofs that cover `bh` were regenerated with it (`proof-bad-bh.jws` is unchanged by design — its `bh` hashes a fixed unrelated string). `expected/verify-results.json` is untouched: it describes the credential layer, not the events.

**Regenerating was the smaller half.** A fixture set that went stale silently once will go stale silently again, so the drift is now a test rather than a habit. `Batch001_StampsTheRegistrysCurrentVersion` asserts every line's `ver` equals `EventTypes.VersionOf(type)` — the generator hard-codes `ver`, so nothing else would notice a bump on one side; `Batch001_CoversEveryRegisteredType` asserts every name in `EventTypes.All` has a line, so adding a type to the registry fails until the generator emits one and coverage cannot narrow without someone deciding to narrow it. `projector.TestGoldenBatchIsAtTheCurrentVersions` is the Go mirror against `projector.currentVer`, plus the requirement that every type that map overrides appears; it is a package-internal test and needed no production change.

**27 lines rather than 23, deliberately.** Reaching 23 for its own sake would have pinned one arbitrary payload per type and pinned nothing about the rules those payloads obey. What matters is *shapes*, so four types carry more than one line: `telemetry.window` with `peak_g` / `max_q_pa` / the `radar_alt_m` aggregate all present and again with all three absent at 1000× warp (a consumer that reads an absent optional as `0` passes the first and fails the second — that pair is the omit-don't-zero rule, made testable); `flight.started` crewed with `kids` populated and `lat` / `lon` present, and an uncrewed probe with `kids` `[]`, `stage_count` 0 and no position; `flight.ended` on `recovered`, on the silent-removal safety net (`despawned`, `crew_count` 0, `body: "unknown"`), and on `destroyed`; and `vehicle.docked` / `vehicle.undocked` for `other_flight` null against `other_flight` a ULID, the taxonomy's one in-payload null. All three envelope `flight` cases are present for the same reason.
...
### PROJ-048 — Four public read endpoints added for the three reader journeys (own stats / global stats / compare with friends), all under the §4.8 cache discipline

*Accepted · 2026-08-07 · WP-READ.*

Four public read endpoints added for the three reader journeys (own stats / global stats / compare with friends), all under the §4.8 cache discipline. `GET /v1/players?q=` (handle search), `GET /v1/players/{handle}/events` (raw event browsing), `GET /v1/compare?handles=a,b,c` (N-handle comparison), and two fields on the existing `GET /v1/players/{handle}` rows — `ascending` and `players` — so a profile can render "#3 of 41" and say which way the board reads without also fetching the board index. Shapes are in [ingest-api.md](ingest-api.md) §4.8. Nothing new in `Deps`, so `catlogd`'s wiring is unchanged; every route goes through `readapi.Server.Register`, which is the only thing that attaches the cross-origin headers (readapi/cors.go), and every response carries `Cache-Control: public, s-maxage=30, stale-while-revalidate=300` including the 400s and 404s.
...
### PROJ-056 — `server/internal/units` is the single definition of what a catlog number looks like

*Accepted · 2026-08-07 · WP-READ.*

The API publishes raw numbers in the unit the event carried, so formatting happens in the renderer rather than on the wire, and it happens in exactly one place so that the same record cannot read two ways. The rules: non-finite → `—`; **three significant figures** (`decimals = clamp(2 - floor(log10 |x|), 0, 6)`) with trailing zeros trimmed and thousands grouped by U+202F; rounding defined on the magnitude as `round(|x| · 10^d) / 10^d` with halves up and the sign re-applied, **specifically** so Go's `math.Round` and JavaScript's `Math.round` cannot disagree at a tie the way `strconv.FormatFloat` and `Number.toFixed` do; `m`/`J`/`Pa` scale by SI prefix (1, k, M, G, T; nothing below the base unit); **`m/s` never scales**, because every speed board is in m/s, this audience reads 7 799 m/s directly, and a per-value scale would mix units inside one leaderboard column; `s`/`ms` become two-component durations (`450 ms`, `37.5 s`, `5m 13s`, `1h 01m`, `243d 01h`, `1y 5d`, a year being a flat 365 days). `units.ForKey` maps a payload key to its unit and exists mainly to hold one trap: **`_ms` is metres per second in every §4.2 payload while the board unit string `"ms"` is milliseconds.** `units.Conformance` is the table that pins the rules, asserted by `units_test.go`; **a rule may not change without the row that fixes its output changing in the same commit**, and anyone rendering catlog numbers from the public API reproduces the table rather than the prose. `web/templates.go`'s `value` and `unit` funcs delegate here rather than carrying a second implementation.
...
### PROJ-078 — The U+202F group separator is gone; grouping is `Intl.NumberFormat` in the reader's own locale

*Accepted · 2026-08-08 · WP-CENSUS.*

The U+202F group separator is gone; grouping is `Intl.NumberFormat` in the reader's own locale. `units` used a narrow no-break space between thousands, chosen because it does not wrap or widen a column. It is also, for practically every reader, not a thousands separator: en-* writes `1,234,567`, de-* writes `1.234.567`, de-CH writes an apostrophe, and the one locale U+202F is actually right for is fr-FR — which is to say catlog was showing several hundred locales one locale's answer. The server renders a canonical rendering and the browser re-renders it in the visitor's locale (PROJ-079). **A separator swap would not have been enough**, which is why this is a re-render rather than a `replaceAll`: en-IN groups `12,34,567` and es-ES leaves `1234` alone, so the *group sizes* differ, not just the character between them. `useGrouping` is left at Intl's `auto` for exactly that reason, and `minimumFractionDigits: 0` preserves rule 2's trailing-zero trim.
...
The cross-language conformance table is now pinned to a locale rather than to a separator. `units.Conformance` asserts the canonical (en-US) rendering, which is exactly what `units.Format` produces — so "the rendered string is checkable character for character" still holds, now qualified by "for the same locale", and a client re-rendering the number in the visitor's locale has a defined starting point. Two new Go tests cover the seam the browser depends on: `Split` reassembles to `Format` exactly, and its `Number` at its `Decimals` round-trips to its own text, so a reader with a script and a reader without one can never see different digits.
...
### PROJ-084 — `/stats` is a page of its own, in the navigation

*Accepted · 2026-08-08 · WP-CENSUS.*

Headline tiles, the four rolling windows each broken down by type, a 90-day daily series, every event type with its share, and the collection census. Deliberately not a leaderboard — no records, no ranking, nobody's handle — and deliberately its own page rather than a front-page band: it is reached on purpose rather than landed on. The daily series draws only days that carried an event, because a day catlog was switched off is not a zero anybody measured.
...
### PROJ-089 — `stats/situation.go` is a port of the mod's table, and it is catlog's one two-sided table
...
**The consequence is a maintenance rule.** This is the only table in catlog with two sides, and a row changed on one side and not the other is a silent divergence between two implementations that are supposed to agree: a board lands on one side of a landing and not the other. Both copies are total by construction — an unknown name reports no contact rather than guessing — so a ninth situation from a future build stays *off* a board instead of arriving on one under the wrong reading.

### PROJ-090 — A new decoder or fold becomes retroactive by rebuild, and no backfill script is written

*Accepted · 2026-08-09 · WP-BOARDS.*

`docs/event-details.md`'s fifth rebuild-versus-incremental divergence is "a build that gained a decoder folds on rebuild what it skipped before", and it is the one that will bite hardest in operation. A fold added today cannot score an event the checkpoint has already passed — the cursor only moves forward — so a newly added board is short by whatever history preceded it until a rebuild runs.

**No backfill mechanism exists and none should.** A projection is rebuildable by definition (D8, D22) — that is the property the whole two-file split exists to buy — so a one-off backfill script would be a second, less-tested implementation of `catlogctl rebuild` that has to be kept level with the folds forever. Rebuild is already admin-triggered, already nightly, already the correctness backstop, already tested to reproduce the incremental result. Using it is cheaper and the failure mode is better: a backfill that disagrees with the folds is a wrong number nobody notices, while a rebuild that disagrees is the thing `TestSeedIsWhatARebuildProduces` fails on.

**The limit, stated so it is not mistaken for a bug.** A rebuild replays what the log holds; it cannot invent a key no payload carried. A fold reading a field that is absent or zero on the stored rows decodes the same absence or the same zero on both paths, and the `> 0` gates refuse it on both.
...
The same audit turned up something worse and unrelated to this change, recorded in `event-details.md`'s known drift: `kitten.kia` and `kitten.tumble` were *also* `flight: null`, so the rebuild's KIA index could never hold anything and the `tuning` flag — which `stats/flight.go` documents as existing specifically to protect `kitten_tumbles` — protects nothing in practice. Both fire only in tests, which construct the events with a flight. Recorded rather than quietly dropping two rows of the suppression matrix; the fix is on the mod side.

**That last paragraph is superseded (2026-08-09):** both events now name a flight — MOD-073, PROJ-092. The four boards this entry is actually about are unchanged, and so is its reasoning: `roster.snapshot` and `kitten.eva_end` still carry no flight, and server-side attribution is still the thing that will not be faked.

### PROJ-092 — Rebuild pass 1 indexes only an attributed `kitten.kia`, and the tests that hid the gap

*Accepted · 2026-08-09 · WP-BOARDS.*

The mod attributing `kitten.tumble` and `kitten.kia` to a flight (MOD-073) is what makes two server-side mechanisms able to fire at all: `scoreable`'s flag gate, which cannot exclude an event that names no flight, and the rebuild's ±2 s KIA window, whose index is keyed by flight. Neither needed new code — they needed input shaped the way they were always written for.

**`rebuild.go`'s condition indexes narrowly, deliberately.** Pass 1 indexes only a `kitten.kia` that has a flight and a sim time. A null-flight KIA is one the mod could not attribute, and there is no key it could be indexed under that is not a guess: session-wide or career-wide would void impact records on flights that had nothing to do with the death. **A missed disqualification costs one record that should not stand; a wrong one voids an innocent player's record and cannot be appealed.** The window fires because the *attributable* KIAs carry a flight, which was the broken half all along. A comment in pass 1 says so, because the condition reads like an incidental guard and is not one.

**What this fixes, and why it went unnoticed.** `event-details.md`'s known drift item 26 and the tail of PROJ-091 recorded both mechanisms as inert. They had passing tests the whole time — `TestRebuildAppliesTheKIAWindow` built the event with a flight the mod did not send. The replacements are shaped like real history instead: `TestRebuildAppliesTheKIAWindowToAScuttleShapedHistory` puts the KIA *after* `flight.ended` with `reason: destroyed` and asserts that the attributed flight loses both impact rows while a second flight, whose death is flightless, keeps its record; `TestTuningFlagExcludesTheTumblesItWasBuiltFor` asserts a flagged flight's tumbles do not score incrementally *or* after a rebuild, while an unflagged flight's does. **A guard whose test data is shaped differently from real data is not a guard**, and the only defence against the next one is fixtures built the way the producer builds them.

### PROJ-093 — Five new boards, and the registry lockstep that has to land with them

*Accepted · 2026-08-09 · WP-BOARDS.*

Recording a number no board reads is a decision to record it, not a feature (MOD-075…MOD-080). Five boards read the new material — `heaviest_to_orbit`, `softest_landing`, `landings`, `lowest_pass`, `biggest_stack` — taking the fixed set from 35 to 40.
...
**The registry lockstep is the hard blocker, and it is not a formality.** `vehicle.landed` is in `ingest.knownTypes`, and it had to land in the same release as the mod that emits it. Until a type is in that list the server answers **`400 malformed_batch` for the entire batch** on the unknown type — so a mod ahead of its server loses *everything* in the batch, not just the new type. **The mod change and the server change cannot be deployed separately in either order**, and `catlog.sim` / `catlog.loadgen` emit `vehicle.landed`, so `make test-integration` fails loudly if they drift apart. That is the intended failure mode.

### PROJ-094 — Three record boards gate on the value, never on the envelope

*Accepted · 2026-08-09 · WP-BOARDS.*

Three payload keys can arrive at a fold saying nothing: `vehicle.orbit.mass_kg` and `flight.started.stage_count` are written as `0` when the mod could not read them, and `telemetry.window.radar_alt_m` is omitted entirely when no sample carried a terrain reading. Each would otherwise poison a board, so `heaviest_to_orbit` requires `mass_kg > 0`, `biggest_stack` requires `stage_count > 0`, and `lowest_pass` refuses `nil` before it looks at a number.

**The mod delta asked for an envelope test on `heaviest_to_orbit` — a `ver` check — and it was built as `> 0` instead.** The two would be the same predicate, since a real orbital mass is never ≤ 0, so the envelope check buys no correctness. It costs something, though: it would make **one** board out of forty a reader of the envelope rather than the payload, and every future maintainer of `boards.go` would have to know which one and why. Every other gate in that file is a value test. Uniformity here is worth more than a redundant belt.

The same reasoning covers `stage_count`, where a value test is not even redundant: 0 means *both* "read failed" and "genuinely no sequences", and the board wants neither, since neither is a stack anybody built.
...
### PROJ-097 — `landed_bodies` reads `vehicle.situation`, not `vehicle.landed`

*Accepted · 2026-08-09 · WP-BOARDS.*

`vehicle.landed` is the explicit event, it is named for exactly this, and it carries `survived`. Sourcing the board from it is the obvious move. **It was not made**, and the two reasons stand on their own.
...
**Nothing gained.** The two events come off the *same* detection in the mod, in that order, so there is no edge the landing sees that the situation does not — the trade is strictly lossy. And `survived`, the one field the landing adds, answers a question this board does not ask: **a body you crashed onto is still a body you reached**, which is the same reason there is no crew gate on `landings` (PROJ-096). A descent-rate decomposition is `softest_landing`'s material, not this board's.

*(Re-examined 2026-08-09 after a third reason — that switching would empty the board when rebuilding older history — was withdrawn as invented: catlog has never stored an event, so there is no history either source would lose. The verdict is unchanged on the two above, which is why it is recorded this way rather than quietly kept.)*
...
### PROJ-098 — `warp_max` is descriptive only, and a decoded `0` must be read as absent
...
**The decode gap, stated because it is a live trap.** The mod's fallback for an unreadable warp is `1` — real time, never a stopped clock — and the field is non-nullable on the wire. But Go's zero value for it is `0`, so a payload that somehow omits the key decodes as a warp of zero rather than as real time. Nothing reads it today, so this is currently invisible. **The first reader must treat `0` as "absent" at the read site**, because the alternative is a per-type decode special case for one field nobody is reading yet. Noted in a comment on the field, in [event-details.md](event-details.md)'s known-drift list, and here.

### PROJ-099 — Propellant and Δv are deliberately not recorded, and the reason is §8 rather than cost
...
**What was built instead**, and it is the honest version of the same appetite: `vehicle.orbit.mass_kg` next to `flight.started.mass_kg`. The pair gives a mass-delivered-to-orbit comparison — what left the pad against what was still there at the milestone — which is efficiency-shaped, is derived entirely from things the game states directly, and validates nothing. `heaviest_to_orbit` (PROJ-093) is the board.

### PROJ-100 — No migration machinery until there is data to migrate

*Accepted · 2026-08-09 · WP-BOARDS.*

catlog has never been deployed and has never stored an event. When the event set was extended, the work was nonetheless framed as a migration off a previous wire: nine types stamped `ver: 2`, identity upcasters registered on the server, a test asserting that "`ver` 1 rows still fold", and documentation about what a rebuild does and does not recover. **None of it had a referent.** There has only ever been one shape of every event, so every one of those artefacts was a compatibility obligation invented against imaginary data.

**The cost was real even though the data was not.** Seven files carried the fiction — the mod's version map, `projector.currentVer`, the upcaster registry, two board comments, the payload decoders' field notes and the taxonomy table — and the conformance vectors briefly pinned a shape nothing emitted, which is the exact failure INGEST-025 exists to prevent, arrived at from the opposite direction. Every one of those places then had to be read and corrected by someone who had to work out which sentences were describing the system and which were describing a past that never happened. **A document describing a history that does not exist is worse than a missing document, because it is believed** — and an agent or a maintainer reading "a `ver` 1 row decodes as 0" reasonably concludes there are `ver` 1 rows.

**The rule.** Do not bump a version, register a transform, or write a compatibility test until there is stored data that requires it. The event set is simply the current event set; `ver` stays on the wire and stays `1` for everything, because it is a real part of the envelope contract and the place a future change will be recorded.
...
The server-rendered datastar site — `site/` plus `server/internal/web/` — and the public read API the rest of the world reads catlog through. There was briefly a second frontend, a React reader at `/app/`; it is gone, and [UI-057](#ui-057) says why. Entry numbers of the decisions that went with it are not reused.
...
### UI-019 — `GET /v1/feed` and `GET /v1/feed/stream` are the data half of the activity feed, beside `GET /v1/feed/sse`

*Accepted · 2026-08-07 · WP-SPA · re-justified 2026-08-09 ([UI-057](#ui-057)).*

The datastar stream is a *rendering* transport, not a data transport: its frames are `PatchElements` payloads carrying rendered `feed-item` HTML addressed to DOM ids the Go templates own. That is why the server-rendered site gets its live feed for free, and it is why **no other client can consume it** — doing so would mean scraping data back out of catlog's own markup and breaking the next time a template changed. The JSON routes publish the same `store.FeedRow` values: `/v1/feed` is the snapshot (first paint, and the fallback when a stream cannot be held open), `/v1/feed/stream` is `text/event-stream` with one `event: feed` frame per new row. The stream deliberately has **no replay** — a reconnecting client re-reads the snapshot, rather than the server growing a `Last-Event-ID` cursor over a table that is capped at 500 and can lose rows out from under it. `Deps.Feed` is optional, so a Server built without a broadcaster simply has no stream route. Rejected: polling only (a feed that is not live is not a feed), and asking clients to parse the datastar HTML (it makes a template an API). The reader these were first written for is gone; the endpoints are not, because they are the only way anything but catlog's own pages can follow the feed.

### UI-020 — CORS is attached to the read API's routes and to nothing else, and the allow-list is configuration

*Accepted · 2026-08-07 · WP-SPA · amended 2026-08-09 ([UI-057](#ui-057)) — the default allow-list is now **empty**.*

`[cors] allowed_origins` (`CATLOG_CORS_ALLOWED_ORIGINS`) is **empty by default, empty in development and empty in production**, and a real origin belongs in the environment rather than in a file (§11). It originally defaulted to the four loopback origins Vite's dev and preview servers could appear as; those went with the reader that used them. **The capability stays, re-grounded on what it is actually for:** `/v1/leaderboards`, `/v1/players/{handle}`, `/v1/feed` and `/v1/stats` are anonymous public facts under the §4.8 cache discipline, and a browser page on another origin — a community dashboard, a stream overlay, somebody's own scoreboard — is a legitimate consumer that the same-origin policy blocks unless the server opts in. The reader was the first such consumer, not the reason the capability makes sense. Deleting the mechanism would also delete the code that *narrows* it — the `Vary`, the credentials refusal, the startup validation and the test below — and whoever re-added cross-origin reads later would rebuild a weaker version. It costs nothing while unused: with an empty list `cors.allow` returns false for every origin and no `Access-Control-Allow-Origin` is ever emitted. The middleware lives in `readapi` and is applied by `readapi.Server.Register`, so the boundary is mechanical: `/api/*` and `/auth/*` (cookie-authenticated), `/v1/ingest` (§4.5.3, on the /v1 prefix but not a read route) and the loopback admin mux never see it. No response ever carries `Access-Control-Allow-Credentials` — these are anonymous public facts, so there is no per-user answer to leak and no reason to let a cookie ride along. `Vary: Origin` is set unconditionally, including on responses with no allow header, because §4.8 ships `s-maxage=30` and a shared cache without the Vary would turn a narrow allow-list into an accidental wildcard. Origins are exact `scheme://host[:port]` matches; `*`, a trailing slash, a path or a non-http scheme are **startup** failures, since all of them fail silently at request time. `TestCORSCoversTheReadAPIAndNothingElse` in `server/cmd/catlogd/` boots the real wiring and asserts the header on the four read routes and its absence on eleven others including `/v1/feed/sse`. **No local target exercises the allow-list** — there is no `make` target serving a page from a second origin — so those tests are where the behaviour is pinned.
...
The raw event view is linked from the profile and from the dashboard, because the open question §6.2 gates it on is already closed in code. `readapi.Server.scanEvents` excludes flagged flights per page and `flaggedFlights` documents why; the endpoint therefore cannot contradict the privacy page, and `docs/ui-design.md`'s "it should not be linked from a public page" was written against the possibility that it might. It links.
...
### UI-045 — A profile's rank is all-time only, so the "You: #147" strip is hidden on a windowed board rather than showing an all-time rank next to a weekly value

*Accepted · 2026-08-07 · WP-SPA-UI.*

§8.1's gap is real: `readapi.Server.player` reads `player_stat`, and `Projections.StatAhead` has no `player_stat_period` equivalent. Showing the all-time number beside a weekly board would be a **wrong** number rather than a missing one. The strip, the "Your standing" panel and the profile's own ranks are therefore unwindowed and say so; the board pages offer `?period=` because `Board` already supports it. When a period-aware `StatAhead` lands, the strip becomes windowed and nothing else changes.
...
`MinQueryLen` is 2 and a shorter query is a **400**, not an empty 200, so a search box that fires on the first keystroke errors on every single search. The guard belongs in the client's own search function — resolve an empty result rather than issue a request known to fail — and not only at each call site, because a call site added later will forget. The empty state says *"No handles match `xyz`"* — **not** "start with", because the endpoint is prefix-first and *then* substring.
...
The defect: `docs/ui-design.md` §4.4 said the board table keeps the unit in the column header and gave the literal `<th class="value">{{$board.Unit}}</th>`, so the board table did exactly that, and a career-time board — which carries `unit: "ms"` and renders `37.5 s`, `10h 23m`, `243d 01h` — got a column of durations under a header reading `ms`. The spec told the renderer to be wrong, which is why the fix is one rule in `server/internal/units` rather than a patch in a template. **The rule:** rules 3, 4 and 6 all render `value + symbol` and an SI prefix goes *before* the symbol, so `1.82 Mm`, `7 799 m/s` and `6 RUDs` all end in the unit the header names — the header shows it verbatim, in its own case. Rule 5 is the sole exception, because no cell in a duration column ends in `ms` and only some end in `s`; its header names the **quantity**, `Time`. No unit at all → `Value`. **The counting boards keep their label** (`RUDs`, `tumbles`, `orbits`, `bodies`, `dockings`, `stagings`, `kittens`) because the label *is* the name of the thing counted, which is exactly what a header wants, and it is what every cell already ends in — the question was asked and the answer is "do not invent `Count`". `units_test.go` asserts the rule *mechanically* as well as by table: render a value in the unit and check whether the string ends in the label, which must be true for every family and false for `Time`. **`units.Measured` is the prose sibling** for the "Measured in ___." line above every board, and it deliberately keeps the storage unit the header drops — *"Measured in ms, shown as a duration."* — because that sentence is the one place a reader is told the API publishes milliseconds, which is what makes `data-value="537500"` and `title="537500 ms"` legible instead of mysterious. **The header cell is the one header cell that is not uppercased**: `M/S` is not a unit, `PA` is not a unit, and `RUDS` is not how catlog writes that word — `thead th.value { text-transform: none }`. Applied to five surfaces (board table header, the "Measured in" line, the board index column, the comparison row header, and — unchanged — the profile's mixed-unit `Value` column). Verified live against a seeded server: `fastest_to_luna` reads `Time` over `50 s` / `8m 57s`, `biggest_lithobrake_survived` reads `m/s` over `214 m/s`, `rud_total` reads `RUDs` over `6 RUDs`, and `distance_travelled` still reads `m` over `4.21 Mm` / `930 km`.

### UI-050 — `→` U+2192 does *not* fall inside `U+0000-00FF` or `U+2000-206F`, and an earlier reading of the range said it did

*Accepted · 2026-08-07 · WP-UI-UNITS.*

`0x2192 > 0x206F`, so the arithmetic was simply wrong, and `@fontsource-variable/inter@5.3.0`'s `unicode.json` confirms it — **U+2192 is in no subset of the package at all**, latin, latin-ext or otherwise. [UI-026](#ui-026) has it right; `docs/ui-design.md` §2.4 has been fixed in place, since that is a living spec. Three literal arrows were still shipping in rendered HTML (`docs_api.gohtml` ×2, `docs_privacy.gohtml` ×1) and are now `›` U+203A, matching the `.more::after` treatment. The same check found `✓` U+2713 and `▾` U+25BE outside every subset too, rendering from a fallback face at the wrong width; where a symbol is wanted the answer is an inline SVG icon, which depends on no font at all. An e2e assertion now walks five rendered pages and fails on any `→`, and the rule to carry forward is that a `unicode-range` is **the list of glyphs the file contains, not a hint**: check `unicode.json`, do not do the arithmetic in your head.
...
**The fix belongs in the client: `fetch(…, { cache: 'no-cache' })`.** It changes only what that client asks for, so the CDN's SWR — where the header genuinely earns its place, one revalidation serving every visitor — is untouched; and Cloudflare ignores client `Cache-Control` request directives by default, so it cannot be used to bust the shared cache either. `no-cache` rather than `no-store` keeps a 304 available; nothing carries a validator today, so these are full 200s in practice, and the fix if that ever matters is an `ETag` on the server rather than a change on the client.

**The comment that survived longest asserted these headers were "ignored by browsers", and that false premise is the whole reason this went unnoticed.** A wrong explanation is worse than none: it stops the next reader looking.

### UI-056 — `[cors] allowed_origins` is empty in production, because everything catlog itself serves is same-origin

*Accepted · 2026-08-08 · WP-CONTAINER · re-justified 2026-08-09 ([UI-057](#ui-057)).*

Every page catlog publishes is rendered by catlogd behind the same nginx that fronts the read API, so nothing catlog ships needs a cross-origin grant. `infra/ansible/inventories/prod/group_vars/catlog.yml` therefore sets `catlog_cors_allowed_origins: []`, which is strictly safer than any non-empty list.

The original reason was narrower — a second frontend was served at `/app/` on the same origin — and that frontend is gone. The conclusion survives its premise: an empty production allow-list is now the *default* rather than a happy accident of where a bundle was hosted. It is not a claim that the allow-list is dead code. It is the opt-in a deployment sets when a third-party browser reader exists ([UI-020](#ui-020)), and it belongs in `CATLOG_CORS_ALLOWED_ORIGINS` rather than in a committed file (§11).


### UI-057 — The `spa/` React reader is deleted; catlog has one frontend

*Accepted · 2026-08-09 · owner's call.*

**Owner's decision: the second frontend was a dead end, and catlog goes with the server-rendered datastar site as its only one.** `spa/` — the standalone Vite + React + react-aria reader over the public read API — is removed in full: the directory, its lockfile and toolchain, its Make targets, its build stage in `infra/docker/Dockerfile.nginx`, the `/app/` and `/app/assets/` locations in `infra/nginx/site.conf.j2`, and its browser smoke test. This reverses the decision that created it and the decisions that only served it; their entry numbers are retired rather than reused, so a `UI-*` citation in old code or an old commit message still resolves to what it always meant.

**Why, beyond "it lost".** The bake-off was a real experiment and it answered its question: two UI patterns over one API cost roughly double in every dimension that matters and bought a comparison nobody needed after the first look. What ending it buys is concrete:

- **One frontend to keep true.** Every specification in `docs/ui-design.md` had to be implemented, verified and kept in step twice. A stale sentence there is how two frontends came to put a raw unit string in a column header ([UI-049](#ui-049)) — identically, because they were both reading the same wrong paragraph.
- **One set of numbers.** `server/internal/units` was two implementations pinned by a transcribed conformance table. It is now one implementation and the table that pins it; a rule change is two edits in one commit rather than three, and there is no port to drift.
- **One public surface a banned player can reach.** The read API is anonymous and ban-filtering lives behind `web.Read` and `readapi`; a second client meant a second place every redaction, ban filter and `min_players` rule had to be honoured, and a second place for one of them to be forgotten.
- **A smaller thing to deploy.** One nginx asset tree, one build stage, one CI workflow, no `SPA_BASE`/`VITE_CATLOG_API_BASE` matrix, and no deep-link fallback to get right per static host.

**What deliberately survives, and must not be deleted as leftovers:**

- **The CORS mechanism** ([UI-020](#ui-020), [UI-056](#ui-056)). It is a general read-API capability for third-party browser readers, not SPA plumbing. Its allow-list is now empty everywhere by default.
- **`GET /v1/feed` and `GET /v1/feed/stream`** ([UI-019](#ui-019)). Public read API. The reader was a consumer, not the owner.
- **`units.Conformance` and `units.LabelConformance`.** They are the citable form of the formatting contract, and the external test package reaches them through the exported names.
...
### MOD-072 — `catlog.toml` gains an `[events]` table; five types are locked on, refused twice, and `CurrentSchema` was deliberately not bumped

*Accepted · 2026-08-09 · mod.*

A player can now switch off individual event types. `[events]` maps a wire type name to a bool, an **absent key means enabled**, and the table is empty on a fresh install and stays empty until the player puts something in it — writing all 22 keys out would mean every future type silently arriving set to whatever a months-old file happens to say, and would turn a one-line opt-out into a wall of noise. **The full list ships commented out in the header instead**, one line per type annotated with the boards it feeds, because there is no example file anywhere and that header is the only self-documenting surface the config has. `TheHeaderDocumentsEveryRegisteredEventType` holds the block in step with `EventTypes.All`, so a new type cannot be registered without the player-facing list gaining a line, and it asserts that exactly the five locked types carry the `# locked on` marker. The header also shouts about quoting: a bare `telemetry.window = false` under `[events]` is, in TOML, a nested table named `window` inside a table named `telemetry` — not a key with a dot in it — and it fails the parse, which under rule 2 drops the whole file to defaults and leaves the player's edit untouched on disk. That failure mode is pinned by `ABareDottedKeyFailsTheParseAndLeavesTheFileAlone` rather than left to be rediscovered.

**Why anything is locked, and the threat model.** It is MOD-065's, unchanged and restated because it is what bounds this: *the attacker is a player editing a text file.* Someone who recompiles the assembly can do anything and always will be able to; that is explicitly not what this defends against, and no amount of client-side machinery would change it. But `[events]` is precisely the shape of edit MOD-065 exists to close — one line, in a file the mod itself writes and hands the player — so shipping it without a locked set would have handed back the hole that entry spent three enforcement points shutting. `"flight.flagged" = false` is the sharp end: it is not vanity-hiding, it is a one-line text edit that makes teleporting, refuelling, resource-editing and console cheating **score normally and appear publicly**.

The locked set is `EventTypes.AlwaysReported`, and `TheLockedListIsExactlyTheIntegritySpine` asserts it is exactly these five and no others:

| Type | Why it cannot be switched off |
|---|---|
| `flight.flagged` | The cheat vector, and the only one. It is the sole writer of `flight_state.flags`; `scoreable()` is the sole exclusion gate for every board fold, `survivedLoad`/`survivedImpact` read `st.Flagged()` directly, the live feed excludes flagged flights, and `store.FlaggedFlights` is what keeps a flagged flight off the public raw-event views the privacy narrative promises it will never appear on. There is no second mechanism and no server-side substitute: the server cannot infer a teleport it was never told about. |
| `kitten.kia` | Drives `KIANear` and the ±2.0 s window in `survivedImpact` — the rebuild-only refinement that disqualifies a fatal crash from `biggest_lithobrake_survived` **and** `biggest_impact_energy`. Without it, a crash that killed the crew scores as a survival. |
| `session.started` | The only source of the save-scum rewind mark (`careerFold` → `MarkRewound`), and the pipeline's own session-reset marker on every `SessionLoadedSignal`. |
| `flight.started` | `flightFold` still creates the row via `EnsureFlight` for any flight-bearing event, so the flag exclusion itself survives — but `StartFlight` never runs, so `crew`, `body` and `started_seq` stay unset and `heaviest_launch` / `most_parts` / `biggest_crew` die. The real loss is the contract: the mod goes to unusual lengths (MOD-059's peek semantics in `Flush`, MOD-060's impact drain in `EndFlight`) to guarantee no flight ULID ever reaches the server without a `flight.started`. Disabling the type makes *every* flight a phantom, by configuration. |
| `flight.ended` | `ended_reason` never gets set, so `Recovered()` is always false: on a rebuild `survivedLoad`'s `b.Refined() && !st.Recovered()` clause zeroes `peak_g_survived` and `max_q_survived`, and `recoveredFold` (`kittens_recovered`) and `recoveryFold` (`biggest_recovery`) score nothing at all. It also removes the feed's flight-end row. This one interacts directly with D22 — the divergence it creates is invisible until the nightly rebuild. |

A `false` on any of the five is **dropped with a warning naming the key**, not rewritten to `true`: absent already means enabled, so dropping says the same thing in fewer words and the saved file then reads as exactly what the mod is doing.

**Why two layers rather than one.** The same argument MOD-065 makes, applied to the same class of edit: clamping the config alone only closes the path you thought of. (1) `ModConfig.Normalize` → `NormalizeEvents` drops unknown keys and drops a locked `false`, both with a warning, so the file the player reads back is the truth about their install — that is the courtesy, and it is what stops a player believing they turned something off. (2) `EventTypeFilter.Create` refuses the locked names again, so a filter built by hand — by `catlog.sim`, `catlog.loadgen`, a test, or a caller written next year that bypasses `ModConfig` entirely — still cannot express "stop reporting `flight.flagged`". That is the guarantee. The redundancy is deliberate and is stated in both files, because a guarantee that lives only in the layer nobody reads is a guarantee nobody can check.

**Why the filter sits at `EventPipeline.Add` specifically.** `Add` is the single funnel every envelope from `ProcessFrame` / `ProcessSignal` / `ProcessSignals` / `Flush` passes through, and C# evaluates the argument before the call — so by the time it runs, the flight ULID has been minted, `Tracker.AddFlag` has recorded the flag, the window has closed and `ImpactCorrelator.Destroyed` has been told. **Suppression is therefore late on purpose: dropping the envelope costs one wasted ULID and rewinds no detector state**, so a disabled type cannot change what the *other* types say. `DisablingVehicleRud_DoesNotMakeAFatalImpactLookSurvived` pins exactly that — `vehicle.rud` off, and the impact still reports `survived: false`. Filtering in `Dispatch` would skip the case body and take that bookkeeping with it; filtering in `CatlogRuntime.Append` would put the filter above `catlog.lib` and make the simulator, the load generator and the tests disagree with the shipped mod about what the pipeline emits. `Add` also carries the `IsAlwaysReported` test a second time, redundantly with `EventTypeFilter.Create`, because this is the choke point and the guarantee has to be readable here rather than two files away. `EventPipeline.SessionStarted` is the one path that returns an envelope without going through `Add`; it is safe **because** `session.started` is locked, so there is no configuration that could have suppressed it — and its `<remarks>` says so with the condition attached, so that if the locked list ever loses `session.started` the method is known to need a filter test.

**Why `CurrentSchema` was not bumped.** The key is additive: an old file has no `[events]` table, Tomlyn deserialises the property to its default empty dictionary, and everything behaves exactly as before. A bump would be actively harmful, not merely unnecessary — `LoadOrCreate`'s schema-mismatch path discards the parsed config and returns defaults, so bumping would silently reset **every existing player's `ingest_url`, `credential_path`, sample rate and log level** in exchange for a key none of them had set. `CurrentSchema` is for a change that makes an old file mean the *wrong* thing; this is not one. Recorded here so it is not re-litigated as an oversight.

**What was not locked: `vehicle.rud`.** The strictest reading would add it — switching it off turns "RUDs" into a self-reported zero. It is vanity-hiding rather than score-inflating: the RUD count is the only thing lost, and the `vehicle.impact.survived` verdict is computed client-side by `ImpactCorrelator` *before* `Add` ever sees the envelope, so it stays honest whether or not the RUD is reported. A player choosing not to publish how often they explode is making a preference; a player choosing not to publish that they teleported is not. The line is drawn at "can this suppression make a number better than it was", and `vehicle.rud` cannot.

**Surface.** `StatusWindow` gained an *Event types* row — `all reported`, or `N off in catlog.toml: <names>` in the warning colour — because a player who switched something off months ago and forgot is otherwise looking at an empty board with no visible cause. It reports and does not edit; see [ROADMAP.md](ROADMAP.md) for why. The name list is precomputed into `EventTypeFilter.DisabledList` at construction, since the window redraws every frame and MOD-071's zero-allocation guarantee is not something a status row gets to spend.

**No wire, server or contract change.** Omitting a type from a batch was always legal — only an *unknown* type is rejected — so `docs/events.md`, every `ver` and the conformance vectors are untouched. Tests: 7 in `ModConfigTests`, 10 in the new `EventGateTests` (built as the mirror of `NoTomlKeyCanNameTheFloor`), 5 in `EventPipelineTests`.
...
Seven of the payload's values split two ways. `lat`, `lon` and `radar_alt_m` are `[JsonIgnore(WhenWritingNull)]` nullables — **absent from the object entirely** when unreadable, never `null` and never `0`. `stage_count`, `mass_kg` on `vehicle.orbit`, `warp_max`, and both landing speeds are non-nullable and carry a defined fallback.
...
Without a body on `flight.ended`, a landing site is **unplaceable**: the flight's last `telemetry.window` may be a whole window old and the vehicle may have changed SOI since, so "where did this flight end" would have no honest answer anywhere in the log.
...
The spec named `Vehicle.GetRadarAltitude()` as the source for `radar_alt_m`. **It is not called, and the recorded numbers are identical.**
...
`catlog/catlogd` (the Go server) and `catlog/catlog-nginx` (the proxy plus the site's assets), built by `infra/docker/Dockerfile.*` and run by `infra/compose.prod.yaml`.
...
No new dependency: node's own `zlib` has brotli, and the site build is hermetic by design.
