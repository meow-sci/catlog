...
`contracts/testdata/batches/batch-001.ndjson` was five lines, five types, every one `ver` 1 and none carrying a wire-v2 key — written before `kitten.tumble` / `kitten.kia` gained a flight and before the seven-type wire-v2 wave, and never regenerated after either. Both suites passed against it the whole time. **That is the failure mode worth naming: a conformance layer pinning a shape the implementations no longer emit is worse than no conformance layer, because it reports green while the thing it guards has moved**, and the two teams reading it — Go and C# — each take it as evidence the other side agrees. The batch is now 27 lines covering **23 of 23** registry types at the `ver` each registry stamps today (nine at `ver` 2), and `bh` moved to `9qWlcOUfLwdKlAHKyBSmoYe21h_-F48E-hUqLWC4WQg`, so `batch-001.br` and the four proofs that cover `bh` were regenerated with it (`proof-bad-bh.jws` is unchanged by design — its `bh` hashes a fixed unrelated string). `expected/verify-results.json` is untouched: it describes the credential layer, not the events.

**Regenerating was the smaller half.** A fixture set that went stale silently once will go stale silently again, so the drift is now a test rather than a habit. `Batch001_StampsTheRegistrysCurrentVersion` asserts every line's `ver` equals `EventTypes.VersionOf(type)` — the generator hard-codes `ver`, so nothing else notices a bump on one side; `Batch001_CoversEveryRegisteredType` asserts every name in `EventTypes.All` has a line, so adding a type to the registry fails until the generator emits one and coverage cannot narrow without someone deciding to narrow it. `projector.TestGoldenBatchIsAtTheCurrentVersions` is the Go mirror against `projector.currentVer`, plus the requirement that every type that map overrides appears; it is a package-internal test and needed no production change.
