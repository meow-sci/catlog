...
and C# suites, and asserted byte-identical on regeneration. The 33-line batch covers **all 25 event
types at their current `ver`**, and the C# suite fails if a newly registered type has no line, so
regenerating after any registry or payload change is not optional — it is what the tests are
checking.
