...
func playerProjectionRows(t *testing.T, r *rig, playerID int64, handle string) map[string][]string {
	...
	queries := map[string]string{
		...
		"flight_state":       `SELECT hex(flight_id), player_id, flags, ended_reason, crew, body, started_seq, engine_count FROM flight_state WHERE player_id = %d ORDER BY hex(flight_id)`,
